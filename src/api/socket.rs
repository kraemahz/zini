...
use super::voice::{create_audio_timing_task, AudioEventChannel, AudioData, AudioContext};
...
async fn client_websocket(
    ...
    if instruct_config_tx.send((auth_user, instruct_in_rx, instruct_out_tx)).await.is_err() {
    ...
    spawn(async move {
        while let Some(chat) = instruct_out_rx.recv().await {
            if output_tx.send(FrontEndMessage::InstructMessage(chat)).await.is_err() {
                break;
            }
        }
...
pub fn routes(
    ...
    let audio_stream: mpsc::Sender<AudioEventChannel> =
        router.get_address().expect("Could not get AudioDataChannel").clone();
    let instruction_config_tx: mpsc::Sender<InstructChannel> =
        router.get_address().expect("Could not get InstructChannel").clone();
    ...
    let ws = warp::path("ws")
        ...
        .map(move |auth: AuthenticatedUser, session: SessionWithStore<MemoryStore>, ws: warp::ws::Ws| {
            ...
            (ws.on_upgrade(move |socket| client_websocket(auth, socket, audio_stream, instruction_config_tx)), session)
        })
        .untuple_one()
