...
pub async fn create_project_handler(
    ...
    db_pool: Arc<DbPool>,
    sender: broadcast::Sender<Project>,
...
pub async fn set_active_project_handler(
    payload: ProjectSetter,
    auth: AuthenticatedUser,
    session: SessionWithStore<MemoryStore>,
    db_pool: Arc<DbPool>,
) -> Result<(impl Reply, SessionWithStore<MemoryStore>), Rejection> {
    ...
    project
        .set_active_project(&mut conn, auth.id())
        .map_err(|_| warp::reject::custom(DatabaseError {}))?;
    Ok((warp::reply::json(&project), session))
}

pub async fn get_active_project_handler(
    auth: AuthenticatedUser,
    session: SessionWithStore<MemoryStore>,
    db_pool: Arc<DbPool>,
) -> Result<(impl Reply, SessionWithStore<MemoryStore>), Rejection> {
    ...
    let user =
        User::get(&mut conn, auth.id()).ok_or_else(|| warp::reject::custom(NotFoundError {}))?;
