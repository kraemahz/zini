...
impl Flow {
    ...
    pub fn list<C>(conn: &mut C, page: u32, page_size: u32) -> Vec<Self>
    where
        C: Connection<Backend = Pg> + LoadConnection,
    {
...
impl FlowConnection {
        ...
        C: Connection<Backend = Pg> + LoadConnection,
    {
        ...
        let mut result = vec![];
        for c in cnx {
            let flow_node = flow_nodes::dsl::flow_nodes
                .find(c.to_node_id)
                .get_result::<FlowNode>(conn)?;
        ...
        C: Connection<Backend = Pg> + LoadConnection,
    {
        ...
        diesel::delete(
            crate::schema::flow_node_connections::table.filter(
                crate::schema::flow_node_connections::from_node_id
                    .eq_any(&node_ids)
...
impl FlowExit {
    pub fn create_exits<C>(
        conn: &mut C,
        flow_id: Uuid,
        exit_nodes: Vec<&FlowNode>,
    ) -> QueryResult<()>
    where
        C: Connection<Backend = Pg>,
    {
...
impl Graph {
        ...
        C: Connection<Backend = Pg> + LoadConnection,
    {
        ...
        // Fetch entry point
        let flow = flows::table.find(flow_id).get_result::<Flow>(conn)?;
